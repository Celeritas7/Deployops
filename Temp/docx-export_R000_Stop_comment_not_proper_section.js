  ...
  function sectionXml(model, unitName, section, lang, style, T, P) {
    ...
    Object.entries(section.phaseItems).forEach(([phase, list]) => {
      ...
      list.forEach((it, i) => {
        ...
        if (grp && grp !== prevGrp) rows.push(trow(tc(p([run(grp, { b: 1, sz: 10, color: T.rule, font: F })]), { span: 5, fill: word ? '#f1f1ec' : '#eef1f5', mar: { t: Math.max(40, mt), b: Math.max(40, mb) } })));
        ...
        if (t.cls === 'r' || t.cls === 'w') itemP += p([run(String(strip(tr(lang, t.cls === 'w' ? 'ws_stop_witness' : 'ws_stop_review'))).toUpperCase(), { b: 1, sz: 7.5, color: t.cls === 'w' ? T.wBar : T.rBar, font: FM, spc: 16 })], { before: 20 });
        let notesP;
        ...
        else notesP = p(desc ? [run(desc, { sz: 9.5, color: T.ink2, font: F })] : []);
